...
export class TypeService {
   ...
   */
  async getAllTypes(query: GetTypeDto, user: GetUserDto) {
    ...
    if (query.contractType) {
      where.contractType = Equal(query.contractType);
    }
    const page =
   ...
   */
  async getTypeById(typeId: number, user: GetUserDto): Promise<Type> {
    const entityManager = await this.loadEntityManager(user.dataBase);
   ...
   *
   */
  async createType(type: CreateTypeDto, user: GetUserDto) {
    const entityManager = await this.loadEntityManager(user.dataBase);
    type.authorId = user.id;
   ...
   */
  async deleteTypeById(id: number, user: GetUserDto): Promise<void> {
   ...
   */
  async deleteType(id: number, user: GetUserDto): Promise<void> {
    const entityManager = await this.loadEntityManager(user.dataBase);
