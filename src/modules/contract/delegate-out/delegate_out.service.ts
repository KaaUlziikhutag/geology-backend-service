...
export class DelegateOutService {
   ...
   */
  async getDelegateOutByContractId(contractId: number, user: IUser) {
   ...
   * const Access = await AccessService.getAccessById(1);
   */
  async getVDelegateByUserId(delegateName: string, user: IUser) {
    const entityManager = await this.loadEntityManager(user.dataBase);
    const items = await entityManager.find(ContractDelegateOut, {
   ...
   *
   */
  async createViewUser(delegateOut: CreateDelegateOutDto, user: IUser) {
    const entityManager = await this.loadEntityManager(user.dataBase);
