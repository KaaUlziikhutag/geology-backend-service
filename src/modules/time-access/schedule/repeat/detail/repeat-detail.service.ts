...
import { EntityManager, Equal, FindManyOptions, In, MoreThan } from 'typeorm';
...
export class RepeatDetailService {
   ...
   */
  async getAllRepeatDetail(query: GetRepeatDetailDto, user: GetUserDto) {
    ...
    const [items, count] = await entityManager.findAndCount(RepeatDetails, {
      where,
      order: {
        createdAt: 'DESC',
      },
  ...
  async createRepeatDetail(
    repeatDetail: CreateRepeatDetailDto,
    user: GetUserDto,
  ) {
    const entityManager = await this.loadEntityManager(user.dataBase);
    ...
    const newRepeatDetail = entityManager.create(RepeatDetails, repeatDetail);
    await entityManager.save(newRepeatDetail);
    ...
    await entityManager.transaction(async (transactionalEntityManager) => {
      for (const detail of repeatDetails) {
        try {
          ...
          const allUserIds = this.combineUserIds(detail.viewUserIds, workerIds);
          if (allUserIds.size > 0) {
   ...
   */
  async deleteRepeatDetailById(id: number, user: GetUserDto): Promise<void> {
   ...
   */
  async deleteRepeatDetail(id: number, user: GetUserDto): Promise<void> {
    const entityManager = await this.loadEntityManager(user.dataBase);
