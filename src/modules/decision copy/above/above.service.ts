...
import { AccessType, ContractState } from '../../../utils/globalUtils';
...
export class AboveService {
   ...
   */
  async getAllAboves(query: GetAboveDto, user: GetUserDto) {
   ...
   */
  async getAboveById(aboveId: number, user: GetUserDto): Promise<Above> {
    const entityManager = await this.loadEntityManager(user.dataBase);
   ...
   *
   */
  async createAbove(above: CreateAbovetDto, user: GetUserDto) {
    const entityManager = await this.loadEntityManager(user.dataBase);
    above.authorId = user.id;
    above.comId = user.companyId;
  ...
  async updateAbove(
    ...
    const existingAbove = await entityManager.findOne(Above, {
      where: { id: id, comId: user.companyId },
   ...
   */
  async deleteAboveById(id: number, user: GetUserDto): Promise<void> {
   ...
   */
  async deleteAbove(id: number, user: GetUserDto): Promise<void> {
    const entityManager = await this.loadEntityManager(user.dataBase);
